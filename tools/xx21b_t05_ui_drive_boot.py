#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""XC-XXI-B T0.5 三态·TASK-026（变体组合深观测 + T0.5 三态尝试，战役最接近 FULL 的一票）：
    深观测趟（只观测不触发 Run）+ Run 触发趟（CreateRemoteThread Run，三态判定）。
fork 自 tools/xx21b_t05_ui_drive_pcell.py (TASK-021/022 版)；原四套脚本（T017/T018/T019/pcell）不改。

判定对象 = "rev2 宿主 (a852880a) + 自举变体 core.dll (7b470117, EP=0x8a0108, ImageBase 0x7ffc57e30000) + config.ini (cde9be13)" 组合。
保留 pcell 版全部机制（泵线程全量即时消费 / CREATE_THREAD 调试句柄 RIP 采样不 OpenThread / EXCEPTION 全记录 /
泵健康自证 / attach_changed_behavior 上报 / sha fail-closed / 导出动态解析 / 防火墙只读核实 / NO_BYPASS=1 /
基址硬门 / run_head 明文预检 / .winlice 残留标注 —— 基址硬门 / run_head 明文预检 / 三态语义 / .winlice 残留标注零改动）。

本版改动（TASK-026 票面，最小 diff，全部披露）：
  1) 常量: DEPLOY -> lab/xx21b_boot; CAND_SHA -> 7b470117... (变体); 标签/docstring; CAND_PREFERRED_BASE -> 0x7ffc57e30000 (文档对照,
     T021 硬门判据仍用 read_candidate_preferred_base_disk() 动态读);
  2) 最小观测扩展 (全部以新代码段/新函数追加, 不改判定逻辑):
     a. run() 新增 --deep 模式: 深观测趟 — 180s 窗口轮询 (每 5s FindWindowW PigToGoLicenseDialog 精确核对 +
        EnumWindows) + 每 2s 全线程 RIP 采样 (调试句柄, CONTEXT 0x100001) + 每 30s 记录线程数/CPU 态;
        观测期满或窗口出现即返回 (进入 Run 触发段由另一趟负责);
     b. run() Run 触发趟新增前置深观测段 (180s, 参数覆盖 OBS_SECONDS): 同 a 的观测逻辑, 期满或窗口出现
        即进入 Run 触发段;
     c. 全线程 RIP 采样 (sample_all_rips): 所有已知线程调试句柄逐个 SuspendThread+GetThreadContext+ResumeThread,
        每 2s 一轮, 每线程每轮记录 {t, tid, rip, owner, rsp, rax}; 新增 rip_log_all 与 deep_observations 累积;
     d. 线程数/CPU 态: 每 30s 记录 threads_seen 数 + 进程 CPU 时间 (GetProcessTimes, FILETIME);
     e. 窗口发现: 复用 find_windows() (EnumWindows + FindWindowW 回退), 每 5s 一次, 窗口出现即记录并置 flag;
     f. urlmon IAT 槽 (0x16f300 动态解析) 前后值 + 页级变化: urlmon_slot_va 由 resolve_urlmon_slot_disk() 动态解析
        (不硬编码 0x16f300, 该值仅作文档对照); 页级 = 槽所在页 (0x1000 对齐) 4KB 快照前后 sha256 + 逐字节 diff 统计;
     g. 深观测数据全部入证据 JSON (deep_observations / rip_log_all / thread_count_history / window_poll_history /
        page_diff)。
  3) 三态判定语义与 T017/T018/T019/T021/T022 逐字一致 (classify 零改动); FULL = Run 线程 RIP 落入 urlmon.dll 模块区间
     + 进程存活 + >=2 趟可复现 (由多趟证据链判定); 新阻塞 = RIP 稳定卡新位置 -> STOP; AV = EXCEPTION 或异常退出 -> STOP。

红线: NO_BYPASS=1; 不真联网; 不改防火墙 (只读核实); 样品/产物不外发; 不写 C:\\Windows; 不新增依赖 (仅 ctypes/subprocess/标准库);
      变体/宿主/壳字节零改动; 不输入真实注册码、不绕过授权 (授权流程是行为等价的一部分, 只观测窗口与消息泵)。
"""
...
# 候选固定基址首选 (变体 ImageBase; T021 基址硬门判据必须用 read_candidate_preferred_base_disk() 动态读,
# 本常量仅作文档对照 — TASK-026 票面: 变体 ImageBase = 0x7ffc57e30000 动态读)。
CAND_PREFERRED_BASE = 0x7FFC57E30000
# 旧 (dump 会话) 死区间, 用于 AV 地址 .winlice 残留命中标注 (不变, T021 口径):
#   ntdll  [0x7ffeeb320000, +0x300000)
#   urlmon [0x7ffec48f0000, +0x1dd000)
OLD_NTDLL_DEAD = (0x7FFEEB320000, 0x300000)
OLD_URLMON_DEAD = (0x7FFEC48F0000, 0x1DD000)

# 深观测默认参数 (TASK-026 票面: 180s 窗口枚举 + 全线程 RIP 采样; 参数覆盖 OBS_SECONDS)
OBS_SECONDS = 180          # 深观测段时长 (s), 可用 --obs= 覆盖
OBS_WINDOW_POLL = 5.0      # 窗口轮询间隔 (s)
OBS_RIP_POLL = 2.0         # 全线程 RIP 采样轮间隔 (s)
OBS_THREAD_POLL = 30.0     # 线程数/CPU 态记录间隔 (s)
...
class runtime:
    def __init__(self):
        ...
        self.rip_log = []          # [{t, rip, owner, rsp, rax}]  Run 线程高密采样 (Run 触发段)
        ...
        # ---- TASK-026 深观测扩展 (新增, 判定逻辑零改动) ----
        self.deep_observations = []   # 深观测段主记录 [{t, kind, detail}]
        self.rip_log_all = []         # 全线程 RIP 采样 [{t, tid, rip, owner, rsp, rax}]
        self.thread_count_history = []  # 每 30s [{t, threads, cpu_user_s, cpu_kernel_s}]
        self.window_poll_history = []   # 每 5s [{t, windows, license_dlg}]
        self.deep_window_seen = False   # 深观测段窗口出现标志
        self.deep_window_at = None
        self.page_diff = None           # urlmon IAT 槽页级变化
    ...
    # ================= TASK-026 深观测扩展: 全线程 RIP 采样 + 窗口轮询 + 线程数/CPU =================
    def sample_all_rips(self):
        """全线程 RIP 采样: 对所有已知线程调试句柄 (泵 thread_handles) 逐个
        SuspendThread + GetThreadContext (CONTEXT 0x100001) + ResumeThread。
        返回 [{tid, rip, owner, rsp, rax}] (逐线程上下文快照, 非原子 — 仅供分布观测)。"""
        with self.pump_lock:
            ths = list(self.thread_handles.items())
        out = []
        for tid, ht in ths:
            with self.ctx_lock:
                prev = self.k32.SuspendThread(wt.HANDLE(ht))
                if prev == 0xFFFFFFFF:
                    out.append({"tid": tid, "rip": None, "owner": None, "rsp": None,
                                "rax": None, "error": "suspend_failed"})
                    continue
                try:
                    ctx = CONTEXT()
                    ctx.ContextFlags = CONTEXT_CONTROL_INTEGER_AMD64  # 0x100001 | CONTEXT_INTEGER
                    ok = self.k32.GetThreadContext(wt.HANDLE(ht), ctypes.byref(ctx))
                    if not ok:
                        out.append({"tid": tid, "rip": None, "owner": None, "rsp": None,
                                    "rax": None, "error": "getctx_failed %d" % ctypes.get_last_error()})
                    else:
                        rip = ctx.Rip
                        out.append({"tid": tid, "rip": hex(rip) if rip else "(zero)",
                                    "owner": self.owner(rip) if rip else "(zero)",
    ...
    def _cpu_time(self):
        """进程 CPU 时间 (GetProcessTimes, FILETIME 100ns 单位 -> 秒)。"""
        class FILETIME(ctypes.Structure):
            _fields_ = [("dwLowDateTime", wt.DWORD), ("dwHighDateTime", wt.DWORD)]
        ft_create, ft_exit, ft_kernel, ft_user = FILETIME(), FILETIME(), FILETIME(), FILETIME()
        if not self.proc:
            return None, None
        ok = self.k32.GetProcessTimes(wt.HANDLE(self.proc), ctypes.byref(ft_create),
                                      ctypes.byref(ft_exit), ctypes.byref(ft_kernel), ctypes.byref(ft_user))
        if not ok:
            return None, None

        def to_s(ft):
            return (ft.dwHighDateTime << 32 | ft.dwLowDateTime) / 1e7
        return to_s(ft_user), to_s(ft_kernel)
    ...
    def _find_license_dlg(self):
        """FindWindowW 精确核对 PigToGoLicenseDialog (P-11: EnumWindows 回调在本环境不触发, 改走 FindWindowW)。"""
        hwnd = self.user32.FindWindowW("PigToGoLicenseDialog", None)
        if not hwnd:
            return None
        p = wt.DWORD(0)
        wtid = int(self.user32.GetWindowThreadProcessId(wt.HWND(hwnd), ctypes.byref(p)))
        if int(p.value) != self.pid:
            return None
        return {"hwnd": int(hwnd), "class": "PigToGoLicenseDialog", "pid": int(p.value), "tid": wtid}

    def deep_observe(self, seconds, tag):
        """深观测段 (TASK-026 票面): 180s 窗口轮询 (每 5s FindWindowW PigToGoLicenseDialog 精确核对 +
        EnumWindows) + 每 2s 全线程 RIP 采样 (调试句柄, CONTEXT 0x100001) + 每 30s 记录线程数/CPU 态。
        观测期满或窗口出现即返回。返回 {"window_seen": bool, "window": ...}。"""
        t0 = time.time()
        last_win = 0.0
        last_rip = 0.0
        last_thr = 0.0
        while True:
            ...
            if now >= seconds:
                break
            # 窗口轮询 (每 OBS_WINDOW_POLL)
            if now - last_win >= OBS_WINDOW_POLL:
                last_win = now
                wnds = self.find_windows()
                lic = self._find_license_dlg()
                self.window_poll_history.append({"t": round(now, 1), "windows": len(wnds),
                                                 "license_dlg": lic})
                if lic:
                    self.deep_window_seen = True
                    self.deep_window_at = round(now, 2)
                    self.deep_observations.append({"t": round(now, 2), "kind": "window_seen",
                                                   "detail": json.dumps(lic)})
                    return {"window_seen": True, "window": lic}
                self.deep_observations.append({"t": round(now, 2), "kind": "window_poll",
                                               "detail": "windows=%d license_dlg=None" % len(wnds)})
            # 全线程 RIP 采样 (每 OBS_RIP_POLL)
            if now - last_rip >= OBS_RIP_POLL:
                last_rip = now
                snaps = self.sample_all_rips()
                self.rip_log_all.extend([{"t": round(now, 2), **s} for s in snaps])
                owners = {}
                for s in snaps:
                    o = s.get("owner") or "?"
                    owners[o] = owners.get(o, 0) + 1
                self.deep_observations.append({"t": round(now, 2), "kind": "rip_sample",
                                               "detail": "threads=%d owners=%s" % (len(snaps), json.dumps(owners))})
            # 线程数/CPU 态 (每 OBS_THREAD_POLL)
            if now - last_thr >= OBS_THREAD_POLL:
                last_thr = now
                with self.pump_lock:
                    nthreads = len(self.thread_handles)
                cu, ck = self._cpu_time()
                self.thread_count_history.append({"t": round(now, 1), "threads": nthreads,
                                                  "cpu_user_s": cu, "cpu_kernel_s": ck})
            if self.pump_process_exited:
                self.deep_observations.append({"t": round(now, 2), "kind": "process_exited",
                                               "detail": "exit_code=%r" % self.pump_exit_code})
                return {"window_seen": False, "window": None, "process_exited": True}
            time.sleep(0.2)
        return {"window_seen": False, "window": None}

    def page_snapshot(self, addr):
        """取 addr 所在页 (4KB 对齐) 的完整快照 sha256 + hex 前缀。"""
        page = addr & ~0xFFF
        data = self.rpm(page, 0x1000)
        if data is None:
            return None
        return {"page": hex(page), "sha256": hashlib.sha256(data).hexdigest(),
                "head": data[:16].hex(), "tail": data[-16:].hex()}

    def _finalize_page_diff(self, slot_va, pre_page_sha):
        """触发段末: 取 post 页快照, 与 pre 页 sha 对比 (页级变化 sha256 对比 + 槽值前后对比)。"""
        page = slot_va & ~0xFFF
        post = self.rpm(page, 0x1000)
        if post is None:
            return {"error": "post read failed"}
        post_sha = hashlib.sha256(post).hexdigest()
        off = slot_va - page
        post_slot = int.from_bytes(post[off:off + 8], "little") if off + 8 <= len(post) else None
        return {"slot_va": hex(slot_va), "page": hex(page),
                "pre_page_sha256": pre_page_sha, "post_page_sha256": post_sha,
                "page_changed": pre_page_sha != post_sha,
                "post_slot": hex(post_slot) if post_slot is not None else None,
                "iat_pre": hex(self.iat_pre) if self.iat_pre is not None else None,
                "iat_post": hex(self.iat_post) if self.iat_post is not None else None,
                "iat_changed": (self.iat_pre != self.iat_post) if (self.iat_pre is not None and self.iat_post is not None) else None}
    ...
    # ================= 主流程 =================
    def run(self, out_prefix="t05", dry=False, terminate_at_end=True, deep_mode=False, obs_seconds=OBS_SECONDS):
        t_all = time.time()
        ...
        print("redline sha OK (core=7b470117 variant EP 0x8a0108, host=a852880a, config=cde9be13)")
        ...
        # ============ TASK-026: urlmon IAT 槽页级变化 (触发 Run 前取 pre 页快照) ============
        # 槽地址 = core_base + 动态解析的 slot RVA (0x16f300 文档对照; 实际值动态)。页级 = 槽所在 4KB 页。
        page_pre = None
        if URLMON_SLOT_VA:
            page_pre = self.page_snapshot(URLMON_SLOT_VA)
            self.log_event("urlmon_page_pre", json.dumps(page_pre))

        # ============ TASK-026: 深观测段 (Run 触发前) ============
        # 180s (可用 --obs= 覆盖): 窗口轮询 + 全线程 RIP 采样 + 线程数/CPU 态。
        # 观测期满或窗口出现即进入 Run 触发段 (窗口出现 -> 立即记录, C-10 破解候选, 继续 Run 触发段 + UI 驱动)。
        obs_result = self.deep_observe(obs_seconds, "pre_run")
        self.log_event("deep_observe_done", json.dumps({"window_seen": obs_result.get("window_seen"),
                                                        "window": obs_result.get("window"),
                                                        "obs_seconds": obs_seconds,
                                                        "process_exited": obs_result.get("process_exited", False)}))
        if obs_result.get("process_exited"):
            # 深观测期内进程退出: 走终态收尾 (不触发 Run — 已无意义)
            fin = self.finish() if self.hthread else {"exit_code": None, "still_active": False}
            self._finalize_page_diff_if(URLMON_SLOT_VA, page_pre)
            self.check_freeze("deep_exited")
            self.terminate_host()
            time.sleep(0.5)
            self._cleanup_pump(wait_s=5)
            self._cleanup_handles()
            return {"redline": "DEEP_OBS_PROCESS_EXITED", "exit_code": self.pump_exit_code,
                    "deep_observations": self.deep_observations,
                    "rip_log_all": self.rip_log_all,
                    "thread_count_history": self.thread_count_history,
                    "window_poll_history": self.window_poll_history,
                    "pump": self.pump_summary(), "deploy_check": deploy_check,
                    "page_diff": self.page_diff, "no_bypass": "1"}
        ...
        # ============ 纯深观测趟 (--deep): 只观测不触发 Run ============
        if deep_mode:
            self.check_freeze("deep_mode_end")
            self.terminate_host()
            time.sleep(1.0)
            self._cleanup_pump(wait_s=8)
            self._cleanup_handles()
            return {"schema": "xx21b_t05_run_ui_dbg_boot_deep/v1",
                    "mode": "deep", "deep_observations": self.deep_observations,
                    "rip_log_all": self.rip_log_all,
                    "thread_count_history": self.thread_count_history,
                    "window_poll_history": self.window_poll_history,
                    "window_seen": self.deep_window_seen,
                    "window_at": self.deep_window_at,
                    "modules": [{"base": hex(b), "size": hex(s), "name": n} for b, s, n in self.modules],
                    ...
                    "redline": {"no_bypass": "1", "candidate_sha256": CAND_SHA,
                                "host_sha256": HOST_SHA, "config_sha256": CONFIG_SHA,
                                "network_deny_all": fw},
        ...
        # 8.5) urlmon IAT 槽页级变化 (post 页快照 vs pre)
        if URLMON_SLOT_VA:
            self.page_diff = self._finalize_page_diff(URLMON_SLOT_VA, page_pre["sha256"] if page_pre else None)
            self.log_event("urlmon_page_post", json.dumps(self.page_diff))
        ...
        out = {
            "schema": "xx21b_t05_run_ui_dbg_boot_verdict/v1",
            "case": "xiongxiong_core",
            "work_order": "XC-XXI-B",
            "task": "TASK-026 变体组合深观测 + T0.5 三态 (变体 core 7b470117 + 宿主 a852880a + 调试端口泵)",
            "date_utc": now_utc(),
            "redline": {
                "no_bypass": "1",
            ...
            "deep_observation": {
                "obs_seconds": obs_seconds,
                "window_seen_during_deep": self.deep_window_seen,
                "window_at": self.deep_window_at,
                "deep_observations": self.deep_observations,
                "rip_log_all": self.rip_log_all,
                "thread_count_history": self.thread_count_history,
                "window_poll_history": self.window_poll_history,
                "pre_run_obs_done": True,
            },
    ...
    def _finalize_page_diff_if(self, slot_va, page_pre):
        """深观测趟进程退出路径下的页级变化收尾 (浅封装)。"""
        if slot_va and page_pre:
            self.page_diff = self._finalize_page_diff(slot_va, page_pre["sha256"])
            self.log_event("urlmon_page_post", json.dumps(self.page_diff))
    ...
    def _classify(self, non_boot_exceptions, urlmon_hits, stable_rip, fin, out_result,
        ...
        """三态判定 (语义与 T017/T018/T019/T021/T022 票面逐字一致; 证据源 = 调试端口)。
        T017 票面语义:
...
def selftest():
    """单元自测: 导出解析 (Run/GetAppVersion 非 0) + urlmon IAT 槽动态解析 (期望 = 0x16f300, 变体口径) +
    候选 PE ImageBase (期望 = 0x7ffc57e30000, 变体口径) + EP 字段 = 0x8a0108 (变体口径)。"""
    h = runtime()
    exports = h.resolve_exports_disk()
    run_rva = exports.get("Run")
    getapp_rva = exports.get("GetAppVersion")
    print("SELFTEST exports: Run=%s GetAppVersion=%s" % (
        hex(run_rva) if run_rva else None, hex(getapp_rva) if getapp_rva else None))
    ok = True
    if not run_rva or not getapp_rva:
        ok = False
    slot = h.resolve_urlmon_slot_disk()
    print("SELFTEST urlmon URLDownloadToFileA IAT slot RVA=%s (expect 0x16f300, 变体口径)" % (hex(slot) if slot else None))
    if slot != 0x16f300:
        print("SELFTEST FAIL: urlmon slot != 0x16f300")
        ok = False
    pref, err = h.read_candidate_preferred_base_disk()
    print("SELFTEST candidate ImageBase=%s (expect 0x7ffc57e30000, 变体口径) err=%r" % (
        hex(pref) if pref else None, err))
    if pref != 0x7FFC57E30000:
        print("SELFTEST FAIL: ImageBase != 0x7ffc57e30000")
        ok = False
    import struct as _s
    image = open(CORE, "rb").read()
    e = _s.unpack_from("<I", image, 0x3C)[0]
    ep = _s.unpack_from("<I", image, e + 4 + 20 + 0x10)[0]
    print("SELFTEST candidate EP RVA=%s (expect 0x8a0108, 变体口径)" % hex(ep))
    if ep != 0x8a0108:
        print("SELFTEST FAIL: EP != 0x8a0108")
        ok = False
    if not ok:
...
def main():
    ...
    if "--selftest" in args:
        sys.exit(selftest())
    prefix = args[0] if len(args) > 0 and not args[0].startswith("--") else "t026"
    out_path = args[1] if len(args) > 1 and not args[1].startswith("--") else os.path.join(DEPLOY, "t026_evidence.json")
    dry = "--dry" in args
    deep = "--deep" in args
    terminate = "--no-terminate" not in args
    obs = OBS_SECONDS
    for a in args:
        if a.startswith("--obs="):
            try:
                ...
                pass
    h = runtime()
    out = h.run(prefix, dry=dry, terminate_at_end=terminate, deep_mode=deep, obs_seconds=obs)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=1, ensure_ascii=False)
    # 调试泵日志单独落盘 (泵全量事件)
    pump_log_path = out_path.replace(".json", "_pump.json")
    with open(pump_log_path, "w", encoding="utf-8") as f:
        json.dump({"schema": "xx21b_t05_dbg_pump_log_boot/v1", "date_utc": now_utc(),
    ...
    if dry:
        ...
        print("=== DEEP SUMMARY ===")
        print(json.dumps({k: out.get(k) for k in ("mode", "window_seen", "window_at", "deploy_check", "pump", "thread_count_history")}, indent=1, ensure_ascii=False))
    else:
        print("=== SUMMARY ===")
        print(json.dumps({k: out.get(k) for k in ("result", "deploy_check", "final", "verdict", "freeze_symptoms", "attach_changed_behavior", "page_diff")}, indent=1, ensure_ascii=False))
