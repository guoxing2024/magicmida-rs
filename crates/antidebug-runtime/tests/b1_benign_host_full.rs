// ADR7-B B1 runtime-only full initialization harness (benign host).
...
fn main() {
    ...
    for round in 0..ROUNDS {
        ...
        // B1: attestation JSON validation (fail-closed).
        let att_str = String::from_utf8_lossy(&att2[..written2.min(att2.len())]).to_string();
        println!("B1_ATTESTATION={}", att_str);
        let mut att_ok = true;
        let mut att_reasons = Vec::new();
        for (needle, label) in [
            ("\"initialized\":true", "initialized"),
            ("\"telemetry_channel\":\"ready", "telemetry"),
            ("\"architecture\":\"x86_64", "architecture"),
        ] {
            if !att_str.contains(needle) {
                att_ok = false;
                att_reasons.push(label.to_string());
            }
        }
        if !att_str.contains("\"profile_id\":\"oreans_origin_x64_v1") {
            att_ok = false;
            att_reasons.push("profile_id".to_string());
        }
        if !att_str.contains("\"profile_digest\":\"adr6-profile-digest-oreans_origin_x64_v1") {
            att_ok = false;
        ...
        // B1: observation window - keep process alive, watch for crash.
        println!("B1_OBSERVATION_START ms={}", observation_ms);
        unsafe { Sleep(observation_ms) };
        println!("B1_OBSERVATION_END alive=true");
