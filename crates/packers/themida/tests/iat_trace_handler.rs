...
//!
//! Note: the legacy walk increments `current_slot` before examining values,
//! so the first advance arms slot 1 (slot 0 is never traced). The extraction
//! reproduces that behavior verbatim; tests pin it.
...
fn slot_walk_skips_null_and_real_api_then_traces_vm_slot() {
    ...
    // Legacy walk starts at slot 1 (slot 0 never traced); slot 1 (real API)
    // is a validated skip; slot 2 (VM entry) is armed.
...
fn found_api_resolves_slot_and_finishes_with_single_writeback() {
    ...
    // Single-step resolves the real API at slot 1.
    ...
    match action {
        IatTraceAction::Finished {
            ...
            assert!(writeback, "resolved slot must be written back");
            // Legacy walk never accounts slot 0 (it starts at slot 1), so a
            // non-empty table can never be product_complete. Pinned as-is.
            assert!(!product_complete, "legacy walk leaves slot 0 unaccounted");
            assert!(!aborted);
        }
...
fn hit_vm_resolves_exit_process_once_then_fails_second_time() {
    ...
    let action = handle_trace_step(&mut query, &mut t).expect("step");
    assert!(matches!(action, IatTraceAction::TraceSlot { .. }));
    assert_eq!(t.slot_values[1], query.exit_process);
    assert_eq!(t.resolved_count, 1);
    assert_eq!(t.failed_count, 0);
