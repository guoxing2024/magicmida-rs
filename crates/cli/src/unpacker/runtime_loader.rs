...
impl RuntimeLoader {
    ...
    unsafe fn thunk_call_v2(
        ...
        let (result, _addr, thread_handle) =
            unsafe { self.thunk_call_tracked_with_handle_code(target, args, 60, drain, &thunk) };
        if let Some(h) = thread_handle {
            // SAFETY: h is a valid owned handle from into_raw(); no double
            // close (the guard was forgotten by into_raw).
            let _ = unsafe { windows::Win32::Foundation::CloseHandle(h) };
...
impl RuntimeLoader {
    ...
    pub unsafe fn load_and_initialize_inner(
        ...
        let digest_authority =
            RuntimeAuthorityManifest::digest_authority_for(&self.authority, &identity)?;
...
impl RuntimeLoader {
    ...
    unsafe fn resolve_mida_exports_remote(
        ...
        let image_size = u32::from_le_bytes([pe[0x68], pe[0x69], pe[0x6A], pe[0x6B]]) as usize;
        ...
        let ed_va = module_base.checked_add(exp_rva).ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed("export directory VA overflow".to_string())
        })?;
        let rd3 = unsafe {
            RPM(
                target,
        ...
        let ords_bytes = num_names.checked_mul(2).ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed(
                "export ordinal array size overflow".to_string(),
            )
        })?;
        ...
        if ords_rva >= image_size
            || ords_rva
                .checked_add(ords_bytes)
                .map_or(true, |end| end > image_size)
        {
            return Err(RuntimeLoadError::ExportResolutionFailed(format!(
        ...
        let funcs_bytes = num_funcs.checked_mul(4).ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed(
                "export function array size overflow".to_string(),
            )
        })?;
        ...
        if funcs_rva >= image_size
            || funcs_rva
                .checked_add(funcs_bytes)
                .map_or(true, |end| end > image_size)
        {
            return Err(RuntimeLoadError::ExportResolutionFailed(format!(
        ...
        let funcs_va = module_base.checked_add(funcs_rva).ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed(
                "export function array VA overflow".to_string(),
            )
        })?;
        let rf = unsafe {
            RPM(
            ...
            let mut name_at =
                |name_ptr_rva: usize, out: &mut Vec<u8>| -> Result<bool, RuntimeLoadError> {
                    ...
                    for k in 0..64usize {
                        ...
                        if rc.is_err() {
                            return Err(RuntimeLoadError::ExportResolutionFailed(format!(
                                "remote read export name failed: rva={rva:#x}"
                            )));
                        }
                        if ch[0] == 0 {
                            terminated = true;
    ...
    pub fn resolve_exports_from_buffers(
        ...
        for i in 0..num_names {
            ...
            let name_off = i.checked_mul(4).ok_or_else(|| {
                RuntimeLoadError::ExportResolutionFailed("export name index overflow".to_string())
            })?;
            let name_end = name_off.checked_add(4).ok_or_else(|| {
                RuntimeLoadError::ExportResolutionFailed("export name slot overflow".to_string())
            })?;
            ...
            let name_ptr_rva = u32::from_le_bytes(name_slot.try_into().map_err(|_| {
                RuntimeLoadError::ExportResolutionFailed("export name slot size".to_string())
            })?) as usize;
            if name_ptr_rva == 0 {
                continue;
            }
            ...
            let ord_end = ord_off.checked_add(2).ok_or_else(|| {
                RuntimeLoadError::ExportResolutionFailed("export ordinal slot overflow".to_string())
            })?;
            if ord_end > ords.len() {
                return Err(RuntimeLoadError::ExportResolutionFailed(
            ...
            let ord = u16::from_le_bytes(ord_slot.try_into().map_err(|_| {
                RuntimeLoadError::ExportResolutionFailed("export ordinal slot size".to_string())
            })?) as usize;
            ...
            for (wi, w) in want.iter().enumerate() {
                ...
                let func_rva = u32::from_le_bytes(func_slot.try_into().map_err(|_| {
                    RuntimeLoadError::ExportResolutionFailed(
    ...
    pub unsafe fn remote_shutdown(
        ...
        let shutdown_addr = loaded.exports.shutdown.ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed("MidaAntidebugShutdown missing".to_string())
        })?;
        let args = ThunkArgs {
...
impl V2ParamsBlob {
    ...
    pub fn parse_offsets(&self, blob_base: u64) -> Result<V2Offsets, RuntimeLoadError> {
        ...
        let len = u64::try_from(self.bytes.len()).map_err(|_| {
            RuntimeLoadError::ExportResolutionFailed("v2 blob length exceeds u64".to_string())
        })?;
        ...
        for (i, &c) in self.bytes[dig_hex_start_us..dig_hex_end_us]
            .iter()
            .enumerate()
        {
        ...
        if expected_hooks > 0 {
            ...
            for i in 0..expected_hooks {
                ...
                let entry_off = surf_off
                    .checked_add(i.checked_mul(8).ok_or(
                        RuntimeLoadError::ExportResolutionFailed(
                ...
                let entry = u64::from_le_bytes(
                    self.bytes[entry_start_us..entry_end_us].try_into().unwrap(),
                );
                if entry == 0 {
                    return Err(RuntimeLoadError::ExportResolutionFailed(format!(
                ...
                let rel = entry.checked_sub(blob_base).ok_or(
                    RuntimeLoadError::ExportResolutionFailed(
...
fn u64_to_usize(v: u64, what: &str) -> Result<usize, RuntimeLoadError> {
    usize::try_from(v)
        .map_err(|_| RuntimeLoadError::ExportResolutionFailed(format!("v2 {what} exceeds usize")))
}
...
impl V2PreflightResult {
    ...
    pub fn surface_relative_offset(&self, index: usize) -> Result<u64, RuntimeLoadError> {
        ...
        va.checked_sub(self.blob_base).ok_or_else(|| {
            RuntimeLoadError::ExportResolutionFailed("v2 surface VA below blob_base".to_string())
        })
    }
}
...
pub fn validate_preflight_result(
    ...
    if preflight.blob_base != params_bytes as u64 {
        return Err(RuntimeLoadError::ExportResolutionFailed(format!(
            ...
            preflight.blob_base, params_bytes
        )));
    }
    ...
    if preflight.expected_hooks != expected_hooks {
        return Err(RuntimeLoadError::ExportResolutionFailed(format!(
            ...
            preflight.expected_hooks, expected_hooks
        )));
    }
    ...
    pub fn preflight_local(&self, blob_base: u64) -> Result<V2PreflightResult, RuntimeLoadError> {
        ...
        if offs.expected_hooks > 0 {
            ...
            for i in 0..offs.expected_hooks {
                ...
                let entry_off = offs.expected_surfaces_off.checked_add(idx_bytes).ok_or(
                    RuntimeLoadError::ExportResolutionFailed(
                        "v2 entry offset overflow".to_string(),
                    ),
                )?;
