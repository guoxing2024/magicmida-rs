...
| Field | Value |
|-------|-------|
| Path | `D:\MidaVault\lab\evidence\gto_launcher\r27_nobypass_round0_20260725\gto_unpacked.exe` |
| Role | r27 no-bypass unpacked candidate |
...
`live_r26b_final_newclass\gto_unpacked.exe` sha256 `4d722619…af28` — all five sites **DIFF**.
...
Harness check with candidate=r27: all five sites **PASS** (`no_bypass_patches=PASS`).
...
| Gate | Claim | Reason |
|------|-------|--------|
| natural_execution | **false / FAIL** | r27 AVs on cold start (heap rebasing); `r27_nobypass_cdb_av.log` |
...
**Default (no candidate) result:** overall **FAIL** or **INCONCLUSIVE** depending on evidence false flags; `product_1_0=false`.

With explicit `true:false` evidence blobs, live gates are **FAIL** → overall **FAIL**, `product_1_0=false`.

With candidate=r27 + sealed clean bytes: bypass sites PASS, but live/UI/script still FAIL → still not product 1.0.
