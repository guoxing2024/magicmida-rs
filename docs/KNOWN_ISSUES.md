...
**2026-08-30 TASK-019 增补（C-5 同样命中我们自己的重建 DLL）**：T0.4 的 core 完美候选（3650ea6c，08-29 07:58 重启前的会话里 dump）文件内携带该会话的陈旧 ntdll 指针 `0x7ffeeb426390`（= 旧 ntdll 0x7ffeeb320000+0x106390；总指挥字节搜索 **2 处命中**，首处文件偏移 0x13f810）。当日同会话 S1-S4 全 PASS（同会话不崩），跨会话即爆：T019 实弹中宿主初始化消费该指针（`thread_rax = thread_rip = 0x7ffeeb426390`，call rax 取指 AV）→ 进程死于 core.dll 加载完成前。**宿主 a852880a 无辜**（同文件旧值 0 命中；其 `.bss 0x112c10` = 当前会话 ntdll+0x106390 = `0x7ffd37ae6390`，T017 3/3 + T018 引导 5/5 互证）。**教训**：任何"raw 内存 dump 重建品"（宿主或 DLL）都携带 dump 会话的绝对指针——跨会话使用前必须过会话指针清洗（TASK-004 data_reinit 机器），且 S1-S4 同会话验证不能替代跨会话验证。

**2026-08-30 TASK-021 增补（清洗后 R1 仍 AV；总指挥全域普查 = C-5 真实规模）**：T020/R1 清洗（8 槽，窗口 = 已知死区间：旧 ntdll `[0x7ffeeb320000,+0x300000)` + 旧 urlmon）后实弹 2/2（D-043）：候选以首选基址 0x7ffe1da10000 加载（**BASE-LOCK 实弹确认**），引导沉降期 AV `0xc0000005 @ 0x7ffeeb1ab7b0`（Rip=AV 地址；Rax=0x7ffe1db4c4c0 ∈ .bss 表区；RVA 0x142090 槽值 = AV 地址逐位一致）。总指挥全域普查（0x7ffc-0x7fff，含未对齐）：清洗 8 槽后**仍余 ~230 个 8 对齐陈旧指针、≥8 个旧会话模块区**——0x141e90–0x142120 连续函数指针表 ~70 槽（0x7ffeeb13–0x7ffeeb1a 区，worker 报告的"0x7ffeeb1a 族"只是其尾部）；.bss 0x13f7d8+ 簇 ~110 槽（0x7ffee950–955 区）；0x142170+ 簇 ~27 槽（0x7ffee9ee–9f3 区）；0x7ffeea0f/a12/a8d/a8f/a90 区 ~35 槽；并存在**恰好等于模块基址的句柄槽**（0x7ffeeb130000/0x7ffee9500000/0x7ffee9ee0000/0x7ffeea0f0000/0x7ffeea120000/0x7ffeea8d0000）。全部低于旧 ntdll 底逃逸 T020 窗口——**按已知死区间设计的扫描对未知旧模块区是结构性盲区**（执行无过错，设计需升级：全域普查 → 偏移对齐当前系统 DLL 导出表打分定身份 → 全量重基底）。13 个 `.winlice` 非对齐残留经 R1 后复扫**恰好 13 全未对齐**（T020 记录精确吻合）；另 3 个未对齐旧 urlmon 区疑似命中（.winlice/.boot 加密/明文混合区，存疑不改动）。**结论升级**：逐族打地鼠不可收敛（加密区内不可见尾部不可知）；修复主线路径 = 当前 boot 重产候选（会话指针按构造同会话）或全域清洗 R2（先离线定身份，1 格验证）。
...
**2026-08-31 TASK-023 根因诊断结果（D-047，总指挥独立复算通过）**：int3 退出漏斗断点（NtTerminateProcess/RtlExitUserProcess/ExitProcess + 候选 EP 判别位，全动态解析）3/3 捕获完整退出链：**ExitProcess（ret = msvcrt+0x3e2c9 = `_exit+0x89`）→ RtlExitUserProcess → NtTerminateProcess ×2**；Rcx=0（exit code 0）、R11=0xc0000135（STATUS_DLL_NOT_FOUND，volatile 存疑）、Rbp=0x140112058（宿主 .bss）。**EP 判别位：NOP stub DllMain 被 LdrpCallInitRoutine 正常调用（attach rdx=1 + detach rdx=0，3/3）→ 排除加载器初始化失败**。时间线：DllMain attach 0.1651s → ExitProcess 命中 0.1675s = core 加载返回后 **2.4ms** 宿主即经 CRT exit(0) 退出。验证趟（host_loader 隔离加载）0 漏斗命中 + 存活——断点不改变行为。**剩余未知 = LoadLibrary(core) 返回 → exit(0) 之间宿主的判定逻辑**（断点未覆盖）→ 下一步离线静态分析宿主退出调用点（零实弹）。

**2026-08-31 TASK-024 离线静态分析（D-049，总指挥逐字节复算通过）**：① 宿主 exit 族 = 4 跳板（exit/_amsg_exit/_cexit/abort）+ 4 明文调用点（EP 0x13d4/0x1306/0x13cd/0x1314），守卫 = EP 0x10c9 `[0x112008]==0 → exit(eax)`（own-image PE-valid flag，init 0x11c7 置位）——但 **T023 实际退出栈上无宿主明文帧 → 实际触发点 = 宿主 Themida VM 区（静态不可达）**；② exit(0) 调用者 = 宿主侧 CRT/VM（候选侧三查排除：静态导入 12 模块 / .winlice 动态表 7 项 / 字符串，零 exit 族）；③ **两代宿主 .text 逐字节一致**（137 差异字节全为会话绑定指针）→ 无代码级新增检查，行为差异归因 = 候选状态（.winlice 材料化会话差异）+ 会话数据；④ 老宿主 36043cb4 当前会话因自身 C-5 无法启动。**C-9 现状粒度：宿主 VM 区对沉睡 core（NOP stub DllMain）的查询失败 → CRT exit(0)**；查询内容静态不可知 → 修复需实弹（选项 A 最小自举实验）或收官（选项 B），待老板裁决。

**2026-08-31 TASK-025 最小自举实验（D-051）：C-9 已破解 [已验证]**——修复 = 候选 EP 字段 0x1027c0（T0.4 固化 NOP stub）→ 0x8a0108（原壳入口，4 字节字段改动/3 字节实际差异，变体 7b470117）。实弹：非附加 3/3 宿主**存活 30-60s+ 不再 exit(0)**（对照 NOP stub 3/3 exit 0 @2.4ms——决定性偏离）；泵 2/2 壳入口被 LdrpCallInitRoutine 正常调用（Rdx=1 attach）、退出漏斗 0 命中、无 AV。根因实弹证实 = **T0.4 固化的 NOP stub 阉割壳自举 → 宿主 VM 区查询失败 → exit(0)**。**注意**：修复绑定会话——变体继承 096f3bdf 重锚指针（BootTime 2026-08-30 10:05:53.549），重启后需以当会话重产候选 + 重施 4 字节 EP 改动。

### C-10 宿主 + 自举变体组合存活但无窗口（未达 T017 等价）[已验证，2026-08-31 TASK-025，3+2/5 可复现]

变体（EP→原壳入口）宿主存活 30-60s+（非附加 3/3 + 泵 2/2）但**零窗口**（PigToGoLicenseDialog 未现，EnumWindows/FindWindow 交叉核对）——与原版 core（T017：存活 + 授权验证对话框）尚差一层。壳自举已启动（EP 命中）但 UI 未达：自举未完成/等待循环/分钟级迟到未排除（观测窗 30-60s）。**重要旁证**：泵下 attach_changed_behavior=False、无 WinLicense 对话框 → C-8 反调试未在变体组合触发（对照 T018 原版 core + 泵触发）→ 调试泵与自举变体兼容，T0.5 三态方法论可用。剩余未知需实弹深观测（选项 A）或归档（选项 B），待老板裁决。

**2026-08-31 TASK-026 三态尝试 + 深观测（D-053，总指挥复算通过并修正一处定性）**：深观测 3/3（180s ×3 = 9 分钟）——授权验证对话框**零出现（分钟级迟到排除）**；宿主存活态 = **1 线程在 .winlice VM 区（core+0x324317）单核满速轮询/校验**（CPU 用户态 1:1 线性）+ 其余线程 ZwWaitForWorkViaWorkerFactory 空闲等待。Run 触发 2/2：Run 线程（入口明文执行）**100% 稳定阻塞于 NtWaitForSingleObject**（ntdll+0x160404，RVA 0x1603f0 桩区；worker 报告 0x60404 系丢前导 1 算术错误，'条件变量'定性由总指挥修正作废）；urlmon 0 命中、IAT 槽/页级零变化、无 AV。**三态判定 = 新阻塞（2/2 可复现）；FULL 未达成**。**C-10 定性（修正后）**：壳 VM 轮询循环从不满足 Run 逻辑所等待的内核对象 → 窗口/urlmon 均不可达；剩余未知 = VM 区等待/信号逻辑（静态不可达，黑盒观测已到边界）。修复方向（若重启战役）= VM 内部状态机还原（GVM-0 量级）或授权流程黑盒 Fuzz——均超当前战役授权。

**2026-08-31 外部专家咨询（D-054）**：C-10 表征被外部定性为 SecureEngine "silent tamper response" 典型指纹（VM 化 opaque loop + 永不 signal 的等待对象）；快照重建路线 (a) 判死（环境指纹密钥源料 + 滚动解密/就地擦除 + 句柄绑定，社区长期结论）；(b) VM 追踪 ROI 极差；唯一工程可行 = 按 GetAppVersion/Run 2 导出契约干净重写（(c)）。原文归档 `docs/EXPERT_CONSULT_REPLY_20260831.md`。外证级（未经我方实验独立验证）；总指挥审计修正三处见 D-054（mock 后端方案改泵 API 层截获 / 无码时 Q5 只能大概率 / 等待句柄应为新进程内合法对象、缺 signal 侧）。
