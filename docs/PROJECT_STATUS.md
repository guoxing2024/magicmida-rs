...
下一件最要紧的事是 **TASK-006R4**（一格实弹，**待老板批格**）：带 TASK-013 产出的受控 ini（异常分发链 hook 关闭）重跑重脱壳，看 text-poll 能否**首次**收敛到 dump——那是缺陷 A 路径 A/B 真正可达的时刻。前置：受控 ini 落位 `C:\Windows\scylla_hide.ini`（InjectorCLI 只搜 Windows 目录；系统级副作用，跑完必删）。
...
① **TASK-006R4**（一格实弹，待批：带受控 ini 重跑，看 text-poll 能否收敛到 dump——ini 落位 `C:\Windows` 是唯一生效路径）→ ② T0.5 续跑 → ③ TASK-007。TASK-013 已完成并验收（2026-08-30，零实弹：hook 配置可控可记录 + ini 查找规则实证 Windows-dir-only）。
