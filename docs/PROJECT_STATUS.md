...
下一件最要紧的事是**向老板要 1 格实弹**去复跑 TASK-006R（XC-XXI-B 3/4 → 4/4）——**离线侧已经无待办工单**。
...
再批 1 格复跑 TASK-006R（一格两验：① 缺陷 A 的实弹替换验证，路径 A/B 二分，fail-closed 拒绝出产物也是合法终态；② C-7 中止的实弹观察——恒同环 ~1024 次事件内触发 `guardless constant-AV storm abort`、无 dump、无 `[GOOD]`、日志非 3.5GB 级）→ T0.5 续跑 → TASK-007。
