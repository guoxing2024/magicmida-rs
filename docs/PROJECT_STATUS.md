...
下一件最要紧的事是 **TASK-014 v2（已授权，D-023/D-024）：IAT 回归定位与恢复**。08-28 的 xx 战役已实证端到端跑通（XX-11：IAT 186/186 全解析 + load_no_crash 10/10 零 AV + S4 业务标记 8/8 对齐，vault `xiongxiong_duokai/xx11_attempt_20260828-112236/`）；TASK-006R5（08-30，路径 A，账本 7/4）恢复 dump 可达后测得 IAT 0/201 全零——这是 `18e0349`→`be28951` 窗口内的**回归**（嫌疑：006R 命令缺 `--oep=captured --data-sections` 两 flag，或 8-29 接管日改动），不是能力缺失。修复 = 恢复，不许改松门。
...
① **TASK-014 v2**（已授权 D-023/D-024：IAT 回归定位（flag/代码两假设）→ 恢复 XX-11 端点（186/186 + load 10/10 + S4 8/8）+ 一格实弹 7/4→8/4；工单 `tickets/TASK-014.md`）→ ② T0.5 续跑 → ③ TASK-007。TASK-006R5 已完成并验收（终态路径 A，账本 7/4，D-022）：受控 ini 经 staging 生效后 0 次 AV、text-poll 首次收敛、fail-closed 192 槽拒绝产物；TASK-013 待验风险（壳反检）实测阴性。
