...
① **TASK-020（候选 core 会话指针清洗·方案 A）已授权在飞（D-039，2026-08-30，纯离线零实弹不烧格）**：扫描分类（旧 ntdll 死区间等）→ 外科手术式补丁（裸槽直补当前会话值 / 重定位覆盖槽按语义处理或如实标注）→ 复扫归零 + 补丁最小性对账 + 结构/导出/.text 完好性证明；交付 pointer_map.json（T0.5 harness 部署时 fixup 的输入）。vault 原件 3650ea6c 只读一字节不动。诚实边界：清洗后候选绑定当前会话（跨重启仍死 = C-5 未根治），三态重跑须同 boot。清洗验收后 → 老板批 1 格跑 T0.5 三态。背景链（T017/018/019 三连，字节级亲验）：P-11 环境垫零 → C-8 原版 core 附加即死（P-10 失误部署原版）→ C-5 候选自带 dump 会话陈旧指针（宿主无辜，worker 归因被法证推翻）。其余暂缓：TASK-007 / clippy 基线门（D-031 F3）/ §九 GTO-UI / 推送（30 提交，D-010）。TASK-014（路径 A'）/ TASK-015（路径 B1'）已验收（D-027/D-029）；遗留小项 = mod.rs LifecycleError 归类补专属行为测试、load 探针 cdb 变体（worker 已标注）。
