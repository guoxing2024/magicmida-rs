...
下一件最要紧的事是 **TASK-003 返工 R2**（堵住会把编译失败读成"全绿"的基线门禁脚本；R1 已验收打回：E 前缀编译错误仍被放行）。
...
2. **P0 — TASK-003 返工 R2：堵住 `check_clippy_baseline.ps1` 的软通过。** R1 已验收打回（总指挥注入 E0308 实测：编译错误仍被判成基线通过 exit 0；工单第 4 条改措辞未做）。返工要求已写进 `tickets/TASK-003.md` 文末，R2 验收标准四条（链接失败 / E0308 注入 / OK 路径 / deny 不误杀）。
3. **P1 — 清还 clippy 基线漂移（TASK-008，推送前必做）。** 2026-08-29 实测 HEAD 实际 warn=354 > 基线 349，WO-23 门禁在本地就是红的（G-7）。10 个机械位点已定位到 file:line。可与 TASK-003 R2 并行（两者文件不相交：TASK-008 不碰基线脚本，TASK-003 不碰位点文件）。
