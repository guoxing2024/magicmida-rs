...
① **TASK-019（T0.5 终跑·完美候选路线）已授权在飞（D-037，2026-08-30，实弹 1 格 XC-XXI-B 11/4 → 12/4）**：B1' 宿主 a852880a + 脱壳 core 完美候选 3650ea6c（T0.4 产物，S1-S4 全 PASS，`.text` 静态明文——C-8 扣解密对它结构性失效）+ T018 调试泵 harness → 三态判定（FULL/新阻塞/AV）。**背景纠偏**：T018 的 C-8 障碍源于总指挥派单部署了原版 core（P-10 再犯），老板以"core.dll 我们不是有另一条线解决吗？"点破 T0.4 线；判定语义边界写死（宿主+候选组合、固定基址残留实弹核验、FULL=路径级结论 ≠ 完全行为等价）。前置链：T017（P-11 环境垫零 → 工具性阻塞）→ T018（C-8 AV 5/5）。其余暂缓：TASK-007 / clippy 基线门（D-031 F3）/ §九 GTO-UI / 推送（26 提交，D-010）。TASK-014（路径 A'）/ TASK-015（路径 B1'）已验收（D-027/D-029）；遗留小项 = mod.rs LifecycleError 归类补专属行为测试、load 探针 cdb 变体（worker 已标注）。
