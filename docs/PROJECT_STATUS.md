...
下一件最要紧的事是 **TASK-013**（纯离线，零实弹，可直接派）：把 ScyllaHide 的 hook 选择变成可控、可记录的配置。它是缺陷 A 实弹验证的硬前置。
...
① **TASK-013**（离线：ScyllaHide hook 选择可控可记录）→ ② 再申请一格实弹 **TASK-006R4**（带受控 ini 重跑，看 text-poll 能否收敛到 dump，那才是缺陷 A 路径 A/B 真正有机会到达的时刻）→ ③ T0.5 续跑 → ④ TASK-007。
