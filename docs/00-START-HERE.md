...
**下一票需要老板批 1 格实弹：TASK-006R2**（工单 `tickets/TASK-006R2.md` 已写好，批了直接派）（XC-XXI-B 3/4 → 4/4）。离线侧该修的都修完了：缺陷 A（C-4，TASK-009）+ C-7 风暴终止（TASK-011）+ 阈值加固（TASK-012），三者的**实弹验证都还没做**，而且它们只能在同一次重脱壳里一起验——这一格同时是：① 缺陷 A 修复的实弹替换验证（路径 A/B 二分，fail-closed 拒绝出产物也是合法终态）；② C-7 中止的实弹观察（恒同环 ~1024 次事件内触发 `guardless constant-AV storm abort`，无 dump、无 `[GOOD]`、日志不再 3.5GB 级）。**不批这一格，T0.5 continue BLOCKED。**
...
工单顺序（**串行派发，同一时间只派一单**——D-014）：**等老板批 1 格实弹 → TASK-006R2（同时验缺陷 A 与 C-7）** → T0.5 续跑 → TASK-007。TASK-005/009/010/011/012 已完成；TASK-006R 首跑已收口（BLOCKED，验证点不可达，实弹 3/4）。**离线侧无待办工单。**
