...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-021 已完成-STOP（D-043）**：清洗后候选 2/2 AV（引导沉降期 `0x7ffeeb1ab7b0`，Run 触发前）；里程碑 = 基址硬门实弹确认（BASE-LOCK 坐实）；总指挥普查升格：陈旧指针真实规模 ~230 槽 ≥8 旧会话模块区。**老板已裁选项 B → TASK-022 执行完毕、审计通过-STOP（D-045，账本 14/4）**：任务 1 活体 dump PASS（`fix_hardcoded_addresses` 重锚 3500 指针）；任务 2 S1-S4 PASS + 普查门实质 PASS（0 会话指针；60 违规 = 哨兵/指令立即数/跨字段 RVA/厂商继承四类误读，普查器 v2 五类判别 + 判别力锚点闭环：094f5401 hard=144 FAIL / 096f3bdf hard=0 PASS）；任务 3 新阻塞 STOP——**C-9：宿主+重产候选组合引导期干净退出 exit 0**（3/3 泵 + 3/3 普通启动；三方对照归因清晰），Run 不可达。**里程碑：C-5 对管线产出路径结构性关闭**（会话干净候选随 dump 产出）。**TASK-023 完成并通过审计（D-047，账本 15/4）**：C-9 根因大幅收窄——退出 = 宿主侧 CRT `_exit`→ExitProcess(0)（决策者 msvcrt+0x3e2c9 精确到字节级），发生在 core DllMain（NOP stub）正常返回后 2.4ms；**EP 判别位排除加载器初始化失败**；R11=STATUS_DLL_NOT_FOUND 提示。剩余未知 = LoadLibrary(core)→exit(0) 之间宿主判定逻辑 → **TASK-024 完成并通过审计（D-049，零格）**：C-9 收窄至**宿主 Themida VM 区对沉睡 core 的查询失败 → CRT exit(0)**（静态不可达）；两代宿主 .text 逐字节一致（无代码级差异，D-048 子分支否决）；候选侧退出三查排除。**TASK-025 完成并通过审计（D-051，账本 16/4）——C-9 破解 [已验证]**：变体 7b470117（EP 0x1027c0→0x8a0108 原壳入口，恰 3 字节差异）宿主存活不再 exit(0)，根因实弹证实 = T0.4 固化 NOP stub 阉割壳自举；**新观测 C-10**：宿主存活但无窗口（未达 T017 等价）；C-8 未在变体组合触发（泵兼容，T0.5 方法论可用）。**TASK-026 完成并通过审计（D-053，账本 17/4）——结局 = 新阻塞-C10 定性（2/2）**：深观测 9 分钟零窗口（迟到排除）+ 壳 VM 轮询循环单核满速；Run 线程 100% 阻塞于 NtWaitForSingleObject（D-053 修正 worker 的 0x60404 丢位错误与'条件变量'误定性）；urlmon 0 命中、槽/页零变化、无 AV → **FULL 未达成**。**老板已裁收官（D-055，2026-08-31"先收官A"）：战役关闭归档，账本冻结 17/4，总结 = `docs/XX21B_CORE_PERFECT_CLOSEOUT_20260831.md`**；外部专家佐证快照重建判死（D-054）；B 项目（CORE-REWRITE 干净重写）经老板裁定为**独立项目**不属本项目，证据库只读供其引用。T0.5 六连因果链完整：T017 P-11 → T018 C-8 → T019 C-5 → T020+R1 清洗 → T021 普查升格 → T022 重产 → T023 exit 归因 → T024 静态解剖 → T025 自举破解 C-9 → T026 三态尝试新阻塞。红线重申：不绕过授权，授权框流程是行为等价的一部分。其余暂缓（TASK-007 / clippy 基线门 D-031 F3 / §九 GTO-UI / 推送 45 提交，D-010）。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1)/017/018/019/020(+R1)/021/022/023/024/025/026 已完成。
