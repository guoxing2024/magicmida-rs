...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-021 已完成-STOP（D-043）**：清洗后候选 2/2 AV（引导沉降期 `0x7ffeeb1ab7b0`，Run 触发前）；里程碑 = 基址硬门实弹确认（BASE-LOCK 坐实）；总指挥普查升格：陈旧指针真实规模 ~230 槽 ≥8 旧会话模块区。**老板已裁 D-043 选项 B → TASK-022（当前 boot 重产候选 + 同会话三态重跑收官，实弹 1 格 13/4 → 14/4，D-044）已派发在飞**——时间硬约束：产物绑定 BootTime `2026-08-30 10:05:53.549`，**重启 = 产物全作废**。配套新工具 `tools/xx21b_session_pointer_census.py`（会话指针普查硬门，自校准 6/6）。T0.5 五连因果链：T017（P-11 垫零→工具性阻塞）→ T018（C-8 原版 core 附加即死）→ T019（C-5 候选陈旧指针）→ T020+R1（清洗 8/8）→ T021（清洗后仍 AV → 全域普查定规模）→ T022（重产根治）。其余暂缓（TASK-007 / clippy 基线门 D-031 F3 / §九 GTO-UI / 推送 38 提交）。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1)/017/018/019/020(+R1)/021 已完成。
