...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-020(+R1) 候选清洗已验收（D-040/D-041，不烧格）——T0.5 三态重跑就绪，等老板批最后 1 格（12/4 → 13/4）**。重跑票面将写入：基址硬门（候选 3549 自引用指针基址锁死，实际加载 ≠ 首选 0x7ffe1da10000 → 触发 Run 前 fail-loud 弃判）、`.winlice` 13 个非对齐残留风险注记、BootTime 连续性自查。背景链（T017/018/019 三连 + T020 清洗，全部字节级亲验）：P-11 环境垫零 → C-8 原版 core 附加即死（P-10 失误部署原版）→ C-5 候选自带 dump 会话陈旧指针（宿主无辜）→ T020 清洗（初版基址错误 = 总指挥 D-038 错误分解被采信 + 非对齐漏扫，R1 纠正 8/8 全对——过程中总指挥两次算术失误记档 D-038/D-041）。其余暂缓（TASK-007 / clippy 基线门 D-031 F3 / §九 GTO-UI / 推送 32 提交）。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1)/017/018/019/020(+R1) 已完成。
