...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-020（候选 core 会话指针清洗，方案 A，纯离线零实弹不烧格，D-039）在飞**——清洗完（复扫归零 + pointer_map.json）→ 老板批 1 格跑 T0.5 三态重跑（harness 已就绪）。T0.5 三连背景链（全部字节级亲验）：T017——非附加下宿主+原版 core 跑到 GUI 层但 RIP 不可得（P-11 环境垫零）；T018——调试泵可采 RIP 但原版 core 附加即死（C-8 扣解密；总指挥当时部署了原版 core 是 P-10 失误，老板一句"另一条线"点破 T0.4 完美候选才是正解）；T019——换完美候选 3650ea6c 后 C-8 消失（Run 字节明文）但候选自带 dump 会话陈旧 ntdll 指针（C-5，宿主无辜，worker 误归因"宿主"已被法证推翻，D-038）→ 宿主初始化消费即 AV。其余暂缓（TASK-007 / clippy 基线门 D-031 F3 / §九 GTO-UI / 推送 30 提交）。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1)/017/018/019 已完成。
