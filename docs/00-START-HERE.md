...
**TASK-004 已完成（2026-08-29）**：T0.7 会话绑定修复的离线闭环补齐——e2e 用例（合成 PE + 新旧 ASLR 表 → cleanup 重写 → 无残留断言）+ schema round-trip + `--help` 可见 `/session-clean`/`/rebase-fixed`，六条验收总指挥亲测全过（含判别力探针红→绿）。
**下一票：TASK-006（原版宿主重脱壳实弹，老板已批 D-011，前置已清）**。注意 worker 自曝的未验证点：`cleanup_artifact` 的 `VA==raw offset` 假定在真实产物上未验证。TASK-005（纯离线复核）可并行。
（推送按老板裁定停在本地，等他逐次确认；推送前建议补跑 `cargo deny check advisories`。）

工单顺序建议：TASK-006 → 解开 T0.5；TASK-005 并行，完成后再开 TASK-007。
