...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-015（待批准后起草：shell trace 线程/时机修复 + 回归定位 + 一格实弹 8/4→9/4）→ T0.5 续跑 → TASK-007**。TASK-005/009/010/011/012/013/006R5/014 已完成；TASK-006R/R2/R3 终态路径 C、R4 终态 STOP、R5 终态路径 A、T014 终态路径 A'（实弹累计 **8/4**，记 D-022/D-027）。
