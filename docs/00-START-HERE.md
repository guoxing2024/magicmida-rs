...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-021 已完成-STOP（D-043）**：清洗后候选 2/2 AV（引导沉降期 `0x7ffeeb1ab7b0`，Run 触发前）→ 按票面 STOP；**里程碑 = 基址硬门实弹确认（候选以首选基址加载 2/2，BASE-LOCK 理论坐实）**；总指挥全域普查升格：陈旧指针真实规模 ~230 槽 ≥8 旧会话模块区（worker 报告的"0x7ffeeb1a 族"只是尾部），T020 按已知死区间设计的窗口属结构性盲区。**当前等老板裁决下一票**：A 全量清洗 R2（离线普查已就绪，1 格验证）/ **B 当前 boot 重产候选（总指挥推荐，结构性消灭 C-5 类）** / C 归档会话绑定边界收官。T0.5 五连因果链：T017（P-11 垫零→工具性阻塞）→ T018（C-8 原版 core 附加即死）→ T019（C-5 候选陈旧指针）→ T020+R1（清洗 8/8）→ T021（清洗后仍 AV → 全量普查定规模）。其余暂缓（TASK-007 / clippy 基线门 D-031 F3 / §九 GTO-UI / 推送 37 提交）。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1)/017/018/019/020(+R1)/021 已完成。
