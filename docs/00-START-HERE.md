...
**推送前置已全部清零（2026-08-29）**：TASK-003（基线门禁软通过，R2 四条验收亲测全过）与 TASK-008（基线漂移清还，三条验收亲测全过，基线 349→337）均已关闭，WO-23 门禁对真基线 exit 0。
**下一票：TASK-004**（T0.7 会话绑定修复的可离线闭环）——唯一被记成"完成"的引擎级正确性缺陷，同时是 TASK-006（老板已批的重脱壳）的前置。
（推送按老板裁定停在本地，等他逐次确认；推送前建议补跑 `cargo deny check advisories`。）

工单顺序建议：TASK-004 → TASK-006；TASK-005 纯离线可并行，完成后再开 TASK-007。
