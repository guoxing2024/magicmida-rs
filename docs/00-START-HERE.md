...
**⚠️ 授权口径问题（D-015）**：这一格是 worker 据「继续」开跑的，而工单和台账都写着"等老板批"。账本如实记为已花（XC-XXI-B **4/4**），口径待老板裁定。根因在我这边——授权状态写在工单状态字段里，对失忆 worker 不可见；已立规：未授权实弹工单**正文第一行**写 ⛔ 硬拦，「继续」不构成授权。
**下一票（需老板批，且账本已超配额）：TASK-006R3** —— **必须先重启机器**：本 boot 合计 **11/11** 次确定性撞同一个 ScyllaHide NtContinue-hook 故障环，同 boot 再试没有证据支持会不同。好消息是重试成本已从"小时级 + 3.5GB"降到 **20ms + 312KB**，几十毫秒就知道答案。工单 `tickets/TASK-006R3.md` 已写好，第一行带 ⛔ 未授权硬拦。
...
工单顺序（**串行派发，同一时间只派一单**——D-014）：**等老板裁定（授权口径 + 是否扩额）→ 重启 → TASK-006R3（验缺陷 A 路径 A/B）** → T0.5 续跑 → TASK-007。TASK-005/009/010/011/012 已完成；TASK-006R 首跑已收口（BLOCKED，验证点不可达，实弹 3/4）。**离线侧无待办工单。**
