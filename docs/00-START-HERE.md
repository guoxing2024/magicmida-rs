...
工单顺序（**串行派发，同一时间只派一单**——D-014；worker 在飞期间禁止派新单——D-026）：**TASK-017（T0.5 续跑）已执行完毕，终态 = 工具性阻塞（D-034，账本 10/4）——B1' 产物 Run 已实调到 GUI 业务层（"授权验证"窗口、3/3 存活），但本环境对非附加式 GetThreadContext 垫零（P-11），RIP 判定不可得；出路已实测（调试端口路径读得真实 RIP）→ 等老板裁定：A' 批 1 格 harness 改调试端口泵重跑（推荐） / B 接受非 RIP 口径（不推荐） / C 挂起转其它战线（TASK-007 / 基线门 / §九 GTO-UI / 推送 21 提交）**。TASK-005/009/010/011/012/013/006R5/014/015/016(+R1) 已完成；TASK-006R/R2/R3 终态路径 C、R4 终态 STOP、R5 终态路径 A、T014 终态路径 A'、T015 终态路径 B1'。
