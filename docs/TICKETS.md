...
| [TASK-006R5](../tickets/TASK-006R5.md) | P1 | 改代码让受控 ini 真正生效（staging 落位 + sha256 校验 + 清理）+ 一格实弹重跑 | 📋 **待授权**（⛔ 需老板批两件：`crates/` 改动授权（`scyllahide.rs` + `helpers.rs`）+ 一格实弹 6/4→7/4；批后首行改授权令牌） | developer | 第一段离线八条（含判别力）全绿；第二段有效尝试强门（配置来源行 + 落位校验行 + `scylla_hide.log` 无 NtContinue/KiUserExc hook 行且 =1 键仍在）达成后，终态四选一如实上报 |
