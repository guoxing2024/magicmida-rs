...
| [TASK-006R3](../tickets/TASK-006R3.md) | P1 | **重启后**再验缺陷 A（路径 A/B）——本 boot 11/11 确定性撞同一 hook 环，必须换 boot | ⏸ 等老板批 1 格实弹（工单已写好，第一行带 ⛔ 未授权硬拦；**重试成本已从小时级降到 20ms/312KB**，见 KNOWN_ISSUES C-6 成本模型更新） | developer | text-poll 收敛到 dump 阶段 + 三个 TASK-009 证据点有命中 |
