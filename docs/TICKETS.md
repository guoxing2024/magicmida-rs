...
| [TASK-026](../tickets/TASK-026.md) | P1 | **变体组合深观测 + T0.5 三态尝试**：深观测趟 ≥2（C-10：180s 窗口枚举 + 全线程 RIP 采样）+ Run 触发趟 ≥2（CreateRemoteThread + 三态语义逐字 + urlmon IAT 槽 0x16f300 动态解析 + 页级变化）；工具 = pcell 常量改动 + 最小观测扩展；基址硬门/明文预检零改动；BootTime 存在性硬门；实弹 1 格 XC-XXI-B 16/4 → 17/4 | 🔧 **已派发待执行**（2026-08-31，记 D-052；老板批选项 A；前提 = 泵与自举变体兼容（D-051 旁证）；结局开放：PARTIAL 重演 / FULL / 新阻塞-C10 / AV） | developer | 深观测数据 + 三态判定 ≥2 趟 + 语义逐字 + BootTime 双查 + 零越界 + vault 证据先行 |
