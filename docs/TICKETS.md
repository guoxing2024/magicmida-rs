...
| [TASK-006R2](../tickets/TASK-006R2.md) | P1 | TASK-006R 二次复跑：一格实弹**同时**验缺陷 A 修复与 C-7 风暴终止（路径 A/B/C 三条终态皆合法） | ⏸ **等老板批 1 格实弹**（批了才派；工单已写好，含第 0 关四条构建字符串门 + 严格停止规则 + 日志留存要求） | developer | 四条字符串命中 + 身份核验 PASS + 明确终态判定（A/B/C）|
