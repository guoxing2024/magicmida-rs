...
| [TASK-003](../tickets/TASK-003.md) | P0 | 堵住 `check_clippy_baseline.ps1` 的软通过 | 🔁 **打回 R1**（2026-08-29：E0308 编译错误被放行 exit 0；打回记录 [runs/20260829-TASK-003-REJECT-1.md](../runs/20260829-TASK-003-REJECT-1.md)） | developer | 编译失败时脚本必须 exit≠0 |
...
| [TASK-008](../tickets/TASK-008.md) | P1 | 清还 clippy 基线漂移（10 个机械位点，推送前必做） | 📋 待领取 | developer | 基线脚本 exit 0 + `TOTAL=349` |
...
| ID | 标题 | 卡在哪 | 解锁条件 |
|---|---|---|---|
| T0.5 | Run UI 事件驱动补测（Run verdict PARTIAL→FULL） | `BLOCKED_ENV`：机器 2026-08-29 07:58 重启后系统 DLL ASLR 重随机化，宿主 `rev2_unpacked.exe` 启动初始化期即 AV，core.dll 从未加载 | 老板已选"重脱壳根治"（D-011）。等 TASK-004 → TASK-006 产出新宿主后续跑。重跑脚本 `tools/xx21b_t05_ui_drive.py` 已就绪 |
