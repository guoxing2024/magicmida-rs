...
| [TASK-024](../tickets/TASK-024.md) | P1 | **C-9 离线静态分析**：宿主 a852880a exit 族调用点全列 → 引导路径 exit 点 + 守卫条件（精确到指令）→ 三方行为自洽解释（原版→对话框 / 094f5401→AV / 096f3bdf→exit0）+ 候选 core.dll 导入表判别 + 老宿主 36043cb4 对照 diff → 修复方向提案（不实施）；**零实弹零格（禁止执行任何样品）** | 🔧 **已派发待执行**（2026-08-31，记 D-048；老板"派这张离线票"；机制假设：EP=NOP stub → 壳不自举 → 宿主查询失败 → exit(0)，待证实/证伪） | developer | exit 调用点全列 + 守卫条件 + 三方自洽解释 + 候选导入表结论 + 老宿主 diff + 零实弹证明 + vault 证据先行 |
