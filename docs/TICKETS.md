...
| [TASK-014](../tickets/TASK-014.md) | P1 | **v2**：IAT 启动路径**回归定位与恢复**（08-28 XX-11 已实证 186/186 + load 10/10 + S4 8/8；`18e0349`→`be28951` 窗口内退化到 0/201——先定位 flag/代码回归源再恢复）+ 一格实弹复现 XX-11 端点 | ▶️ **已授权，待派发（v2 修订，授权包不变：同 8 文件 + 同 1 格 7/4→8/4）**（记 D-023/D-024；授权令牌已写入工单首行） | developer | 第一段八条全绿（回归定位报告 + 完整清单外发 + 判别力）；第二段强门（沿用 R5 staging 四件套）达成后终态 B1'/B2/A'/C/D 如实上报 |
