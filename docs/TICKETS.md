...
| [TASK-016](../tickets/TASK-016.md) | P1 | **阶段收尾**：B1' 能力固化——硬编码审计清除（行为中性，先审后改）+ preflight 能力预检（无 ini 响亮失败）+ 复现 runbook + vault 索引 + 全仓回归快照（**纯离线零实弹**） | 🔶 **条件通过（D-031），补正中**（2026-08-30 worker 交付 + 总指挥审计：8 标准亲验 7 PASS——diff 6 文件全授权、四套件真退出码 0 计数 1054/176/580/**263**、总指挥换缝探针（非零值分支）红→恢复、runbook 七判据对 vault 实证逐条一致、INDEX 抽验 6/34 sha 一致、零实弹；**2 项补正**：F1 `iat_gap_retarget.rs` `1..64`→`1..=64` 未申报窗口加宽（B1' 可证不受影响仍须还原）、F2 硬编码审计漏盘 dump_process.rs 既有 GTO-UI 样品锚区；另 F3 = clippy 基线门**既有**失败（61→62，T016 贡献 0，修门需老板另授权）。补正工单 = TASK-016R1；R1 验收后整单入栈） | developer | 八条验收全绿（含硬编码审计报告含只读文件发现、preflight 判别力、runbook 一致性、全仓 workspace 全绿） |
| [TASK-016R1](../tickets/TASK-016R1.md) | P1 | **T016 审计补正（微型）**：F1 行为还原（`GAP_NAME_NEIGHBOR_MAX_DELTA` 64→63，`1..=` 语义逐位还原 pre-T016）+ F2 审计文档增补（GTO-UI 既有锚区清单入 `HARDCODING_AUDIT_T016.md` §九 + 全称命题改限定 + iat_completeness 路径勘误）；**D-030 范围内无新令牌不耗格，纯离线零实弹** | 🔧 **已派发待执行**（2026-08-30，记 D-031；F3 基线门只记录不修——修门需老板另授权） | developer | 补正后 pe lib 1054 / acc lib 263 不变 + clippy(-D 三 lint)/fmt 0 + diff 仅 2 文件 + 零实弹自证 |
