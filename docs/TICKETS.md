...
| [TASK-006R4](../tickets/TASK-006R4.md) | P1 | 带受控 ini（异常分发 hook 关闭）重跑重脱壳——缺陷 A 路径 A/B **首次真正可达** | 📋 **待领取（已授权 2026-08-30，可直接派）**（XC-XXI-B 5/4→**6/4** 扩额 + `C:\Windows` 落位放行，见 D-018；首行已换授权令牌；前置亲验：BootTime = 2026-08-30 01:28:40 同 R3 boot、起点 HEAD = ca06117） | developer | 受控 ini 真正生效（配置来源行 + hook 行缺失）前提下，终态四选一如实上报（A：TASK-009 门命中 / B：产物+存活探针 / C：环仍在→记 exc 偏移 / D：新形态） |
