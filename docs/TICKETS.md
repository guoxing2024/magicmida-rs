...
| [TASK-019](../tickets/TASK-019.md) | P1 | **T0.5 终跑（完美候选路线）**：B1' 宿主 a852880a + **脱壳 core 完美候选 3650ea6c**（T0.4 产物，.text 静态明文——C-8 对它结构性失效）+ T018 调试泵 harness，三态判定（语义与 T017/T018 逐字一致）；实弹 1 格 XC-XXI-B 11/4 → 12/4 | 🔧 **已授权待派发**（2026-08-30，记 D-037；老板"批准"；背景 = 总指挥 T017/T018 部署选错原版 core（P-10 再犯，老板一句"另一条线"点破）；判定语义边界写死：宿主+候选组合、固定基址残留实弹核验、FULL=路径级结论 ≠ 完全行为等价） | developer | 三态判定（FULL/新阻塞/AV，证据源=调试端口，≥2 次可复现）+ 泵健康自证 + 基址一致性记录 + 零越界（只新建 pcell 脚本） |
