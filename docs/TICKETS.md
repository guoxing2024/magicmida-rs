...
| [TASK-021](../tickets/TASK-021.md) | P1 | **T0.5 三态重跑（收官之战）**：B1' 宿主 a852880a + 清洗后候选 core（094f5401）+ 调试端口泵 harness（sha 门更新 + **基址硬门**：实际 ≠ 首选 0x7ffe1da10000 → Run 触发前 fail-loud 弃判 + run_head 明文预检），三态判定（语义逐字沿用）；实弹 1 格 XC-XXI-B 12/4 → 13/4 | 🔧 **已派发待执行**（2026-08-30，记 D-042；老板"批准"；`.winlice` 13 残留命中 → 标注 STOP；BootTime 连续性自查；FULL = 路径级结论收官） | developer | 三态判定（FULL/新阻塞/AV/附加改变行为，证据源=调试端口，≥2 次可复现）+ 基址硬门记录 + 泵健康自证 + 零越界 |
