...
| [TASK-014](../tickets/TASK-014.md) | P1 | IAT 启动路径重建：192 启动路径站点可解析（XX-10-A 回溯扩展 + 完整站点清单外发 + 逐槽诊断）+ 一格实弹冲路径 B | 📋 **待授权**（⛔ 需老板批两件：`crates/pe` + `crates/packers/themida` 改动授权（清单见工单，门语义不许动）+ 一格实弹 7/4→8/4；批后首行改授权令牌） | developer | 第一段八条全绿（含完整清单外发 + 判别力）；第二段强门（沿用 R5 staging 四件套）达成后终态 B1/B2/A'/C/D 如实上报 |
