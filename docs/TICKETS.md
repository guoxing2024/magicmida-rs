...
| [TASK-006R3](../tickets/TASK-006R3.md) | P1 | **重启后**再验缺陷 A（路径 A/B）——本 boot 11/11 确定性撞同一 hook 环，必须换 boot | 📋 **待领取（已授权，可开工）** —— 老板 2026-08-30 扩额批 1 格（D-016），XC-XXI-B 4/4 → **5/4**；前置"已重启"总指挥亲验 BootTime = `2026-08-30 00:54:51`；工单第一行已换成授权令牌（worker 须回抄）| developer | text-poll 收敛到 dump 阶段 + 三个 TASK-009 证据点有命中 |
