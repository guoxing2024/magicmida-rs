...
| [TASK-023](../tickets/TASK-023.md) | P1 | **C-9 根因诊断**：调试泵 + int3 退出漏斗断点（ntdll!NtTerminateProcess / ntdll!RtlExitUserProcess / kernel32!ExitProcess / kernel32!TerminateProcess，System32 导出动态解析零硬编码）+ 候选 EP 判别位（NOP stub DllMain 是否被调用）→ 命中法证（真地址/模块归属/寄存器/栈链 rsp[0..16]）→ **退出决策者归属表**；验证趟（host_loader 隔离加载预期 0 命中存活）+ 诊断趟 ≥3；诊断 only 不修复；BootTime 存在性硬门（候选绑定本 boot）；实弹 1 格 XC-XXI-B 14/4 → 15/4 | 🔧 **已派发待执行**（2026-08-30，记 D-046；老板"C-9 根因诊断"；诚实结局预定义：0 命中 = 直接 syscall 绕过漏斗（有效结论）；仪器化改变行为 = STOP） | developer | 断点解析零硬编码 + 验证趟/诊断趟证据齐全 + 决策者归属表 + EP 判别位结果 + BootTime 双查 + 零越界 + vault 证据先行 |
