...
| [TASK-020](../tickets/TASK-020.md) | P1 | **候选 core 会话指针清洗（方案 A）**：扫描分类（旧 ntdll 死区间等）→ 外科手术式补丁（裸槽直补/重定位覆盖槽按语义处理）→ 复扫归零 + 补丁最小性 + 结构完好性证明；交付 pointer_map.json；**纯离线零实弹不烧格，vault 原件只读** | 🔶 **条件不通过（D-040）→ R1 微补正中**（执行纪律全 PASS：vault 原件不变、24 字节最小性、.reloc 零重叠、结构完好、零实弹；**两处实质缺陷**：① 7 个 ntdll 槽基址 +0x100000 错误——根因 = 总指挥 D-038 错误分解被 worker 采信，责任在总指挥；② 非对齐扫描未做——总指挥独立扫描发现 13 个 .winlice 残留；urlmon 槽正确） | developer | 复扫归零 + 补丁对账 + 结构/导出/.text 完好 + vault 原件 sha 不变 + 零实弹自证 |
| [TASK-020R1](../tickets/TASK-020R1.md) | P1 | **T020 清洗基址纠正（微型）**：7 个 ntdll 槽按正确基址 0x7ffd379e0000 重补（总指挥提供逐槽期望值表）+ 非对齐普查补漏（13 个 .winlice 残留列入 residual_unpatched 不盲改）+ 全套重验证；**D-039 范围内无新令牌不烧格，vault 原件只读** | 🔧 **已派发待执行**（2026-08-30，记 D-040；含 D-038 勘误：当前会话 ntdll = 0x7ffd379e0000；另发现候选基址锁死（3549 自引用 + 4 条 reloc）→ T0.5 重跑将设基址硬门） | developer | 逐槽值与总指挥期望表一致 + 对齐复扫归零 + 非对齐残留入册 + 全套重验证 + 零实弹 |
