...
| [TASK-012](../tickets/TASK-012.md) | P1 | C-7 修复加固：风暴阈值裕量（32→4 位数）+ 常量拼写 `GARDLESS`→`GUARDLESS` + host 腿 `storm_abort→Err` 补测试 | 📋 待领取（**下一格实弹之前必做**：32 对硬 fail-closed 裕量偏薄——健康侧实测 0 次、风暴侧 20 万–322 万次，误杀会白烧一格实弹） | developer | host 腿映射有直接测试且判别力可证 + 既有 16 用例全绿 |
