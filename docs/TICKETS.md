...
| [TASK-014](../tickets/TASK-014.md) | P1 | IAT 启动路径重建：192 启动路径站点可解析（XX-10-A 回溯扩展 + 完整站点清单外发 + 逐槽诊断）+ 一格实弹冲路径 B | ▶️ **已授权，待派发**（2026-08-30 老板批准两件：`crates/` 改动 + 一格实弹 7/4→8/4，记 D-023；授权令牌已写入工单首行） | developer | 第一段八条全绿（含完整清单外发 + 判别力）；第二段强门（沿用 R5 staging 四件套）达成后终态 B1/B2/A'/C/D 如实上报 |
