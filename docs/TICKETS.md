...
| ID | 标题 | 卡在哪 | 解锁条件 |
|---|---|---|---|
| T0.5 | Run UI 事件驱动补测（Run verdict PARTIAL→FULL） | **旧双重阻塞已解除**（缺陷 A 已修+实弹验证；同 boot 风险暂不适用，D-033）→ **已立项 TASK-017（2026-08-30 授权 1 格，XC-XXI-B 9/4 → 10/4，在飞）**：换治理产物 a852880a + 脚本基址动态化。硬前置链（TASK-009 → 重脱壳 + S1-S4 → T0.5）已由 T015 B1' 满足 | 重跑脚本 `tools/xx21b_t05_ui_drive.py` 已就绪（须按 TASK-017 票面动态化适配） |
