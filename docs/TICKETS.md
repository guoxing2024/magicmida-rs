...
| [TASK-020](../tickets/TASK-020.md) | P1 | **候选 core 会话指针清洗（方案 A）**：扫描分类（旧 ntdll 死区间等）→ 外科手术式补丁（裸槽直补/重定位覆盖槽按语义处理）→ 复扫归零 + 补丁最小性 + 结构完好性证明；交付 pointer_map.json；**纯离线零实弹不烧格，vault 原件只读** | 🔧 **已派发待执行**（2026-08-30，记 D-039；老板"方案a"；清洗后候选绑定当前会话（跨重启仍死 = C-5 未根治，诚实边界入票）；动 crates → STOP 请示） | developer | 复扫归零 + 补丁对账 + 结构/导出/.text 完好 + vault 原件 sha 不变 + 零实弹自证 |
