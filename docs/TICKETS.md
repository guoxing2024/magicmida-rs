...
| [TASK-016](../tickets/TASK-016.md) | P1 | **阶段收尾**：B1' 能力固化——硬编码审计清除（行为中性，先审后改）+ preflight 能力预检（无 ini 响亮失败）+ 复现 runbook + vault 索引 + 全仓回归快照（**纯离线零实弹**） | ▶️ **已授权，待派发**（2026-08-30 老板令"派个单做个阶段收尾，别以后用不能用了。还有要注意通用性，不要有硬编码"；crates/ 授权按工单清单、零实弹不耗格，记 D-030；授权令牌已写入工单首行） | developer | 八条验收全绿（含硬编码审计报告含只读文件发现、preflight 判别力、runbook 一致性、全仓 workspace 全绿） |
