...
| [TASK-019](../tickets/TASK-019.md) | P1 | **T0.5 终跑（完美候选路线）**：B1' 宿主 a852880a + **脱壳 core 完美候选 3650ea6c**（T0.4 产物，.text 静态明文——C-8 对它结构性失效）+ T018 调试泵 harness，三态判定（语义与 T017/T018 逐字一致）；实弹 1 格 XC-XXI-B 11/4 → 12/4 | ⛔ **已起草，等老板授权**（2026-08-30；背景 = 总指挥指出"另一条线"：T0.4 完美候选被 T017/T018 派单遗漏（P-10 再犯，D-037 将记）；候选文件在 vault 快照 sha 现算核对一致；判定语义边界（宿主+候选组合）与"附加改变行为"保险丝已写入票面） | developer | 三态判定（FULL/新阻塞/AV，证据源=调试端口，≥2 次可复现）+ 泵健康自证 + 零越界（只新建 pcell 脚本） |
...
| ID | 标题 | 卡在哪 | 解锁条件 |
|---|---|---|---|
| T0.5 | Run UI 事件驱动补测（Run verdict PARTIAL→FULL） | **TASK-017/018 已执行（2026-08-30，2 格，账本 11/4），终态 = RIP 级判定结构性不可达**：非附加（T017）——B1' 产物 Run 实调到 GUI 业务层（"授权验证"对话框 3/3、GUI 存活、无 AV），但外部 RIP 采样被环境垫零（P-11）；调试附加（T018）——RIP 采样可用（真实 Rip），但 WinLicense 反调试扣住 `.text` 解密 → Run 入口即 AV 5/5（**C-8**）。**观察与执行结构性互斥** → Run verdict 诚实停在 PARTIAL，附"不可达性证明"（对照实验对在 vault 可复算）。剩余路线 = 非托管环境机器（脚本已就绪）或壳进程内自证（红线外）——待老板裁定是否投入 | 两套 harness（T017 外部版 / T018 调试泵版）均已入栈，可复算 |
