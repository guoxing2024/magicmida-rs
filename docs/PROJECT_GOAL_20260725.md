...
### 时光一键宏.exe（origin_macro）— **完美脱壳达成 ✅ (2026-07-25)**

全部维度达标 + 实证可修改：

| 证据 | 结果 |
|------|------|
| 结构 R0B | StructuralPassBehaviorPending |
| 加载 10× 隔离 | 10/10 |
| `.text` 熵 | 6.045（明文代码，非加密） |
| x64 prologue | `sub rsp`×51、`push rbp`×113（可读代码） |
| 产品字符串 | `授权验证`/`授权码` GBK 明文在 .rdata |
| **可修改性** | 改 1 字节后 R0B 仍过 + load 仍 Pass + business_dialog 仍 Pass（无完整性校验锁死） |
| 行为等价 | license 拒绝路径双侧 N=3 Pass + status 消息一致 |
| 零绕过补丁 | 无 |
| 可复现 | 当前 CLI fresh dump 全绿 |

**结论**：脱壳层目标达成。唯一测不了的 valid-code 接受路径属于许可层，不属于脱壳层（无有效授权码，与脱壳完整性无关）。
