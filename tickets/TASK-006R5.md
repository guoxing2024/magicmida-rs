...
⛔ **未授权硬停：本单含①`crates/` 代码改动 与②一格实弹。未获老板批准前不许执行任何实弹步骤——「继续」/「go on」不构成授权（D-015）。批准后总指挥会把本行改写为授权令牌，你必须在报告第一节原文回抄该行。**

- **优先级**：P1（缺陷 A 实弹验证的硬前置；R4 已实证这是唯一路径）
- **状态**：📋 待授权（等老板批：`crates/` 改动授权 + 一格实弹）
- **岗位**：developer（第一段离线改代码，第二段实弹验证）
- **账本**：XC-XXI-B **6/4 → 7/4**（需老板明确再扩一格）
