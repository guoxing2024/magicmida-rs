...
- **前置**：机器已重启 —— **已满足**（总指挥亲验，见上方令牌）。你仍须自己贴一次 BootTime 复核，若与令牌里的值不一致（说明又重启过或环境变了），照实记录后继续，不用 STOP。
...
1. **机器已重启**：贴 `BootTime`。总指挥亲验值 = `2026-08-30 00:54:51`（上一 boot 是 `2026-08-29 07:58:23`，11/11 确定性撞环的那个）。**前置已满足**，你贴一次复核即可，不用 STOP。
2. **构建核验**：`git log --oneline -1` 的 HEAD 必须含 `e5494c8`（TASK-006R2 收尾提交）；MSVC 环境（`tools/_enter_msvc_env.cmd`；**Git Bash 的 link.exe 会遮蔽 MSVC 链接器，必须用这个**）构建 release；exe 里四条字符串各须命中：
   - `zero-filled IAT region`
   - `TASK-009 fail-closed`
   - `guardless constant-AV storm abort`
   - `C-7: guardless constant-AV storm`

   任一条不命中 → **STOP 上报**，不许开跑。贴构建日志与四条命中的原始输出，把 exe 的 sha256 与体积记进报告。

   **省时提示（总指挥已核实）**：`5064363..e5494c8` 之间 `crates/` / `Cargo.toml` / `Cargo.lock` **零改动**（只动了 docs、tickets、runs），所以 TASK-006R2 那个已核验过的 exe 仍是当前代码：`D:/MidaVault/lab/evidence/xx21b_006r2/mida-cli_006r2_built.exe`，sha256 `0c407a9722d23ca2796479ffc3f32a553b77ba64d25e82c1d9c28de19dc12829`，5795328 B。你**可以直接复用它**省掉一次 release 构建，但报告里必须贴：① `git diff --stat 5064363..HEAD -- crates/ Cargo.toml Cargo.lock` 为空的原始输出，② 复用 exe 的 sha256 复算结果，③ 四条字符串命中。想重新构建也完全可以，照原流程走。
